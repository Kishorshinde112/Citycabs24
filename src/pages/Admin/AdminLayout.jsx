      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-8 shrink-0 shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={toggleNotif}
              className={`px-2.5 py-1.5 rounded-xl transition flex items-center gap-1.5 cursor-pointer text-xs font-bold ${
                notifOn
                  ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200'
                  : 'bg-slate-100 text-slate-500 hover:bg-slate-200 border border-slate-200'
              }`}
              title={notifOn ? 'Notification Sound is ON (Click to turn off)' : 'Notification Sound is OFF (Click to turn on)'}
            >
              {notifOn ? (
                <>
